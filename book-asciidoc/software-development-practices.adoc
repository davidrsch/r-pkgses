...
Como lo sugiere el menú interactivo, `+usethis::use_github_action()+` le brinda acceso a flujos de trabajo prediseñados distintos de `+R CMD check+`. Además de las opciones destacadas, puede usarlo para configurar cualquiera de los flujos de trabajo de ejemplo en https://github.com/r-lib/actions/tree/v2-branch/examples#readme[`+r-lib/actions+`] pasando el nombre del flujo de trabajo. Por ejemplo:
...
* Construir el sitio web de su paquete e implementar el sitio renderizado en GitHub Pages, como se describe en <<sec-website-deployment>>. Véase también `+?usethis::use_pkgdown_github_pages()+`.

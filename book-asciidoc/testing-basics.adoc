...
Esta expectativa tiene como objetivo probar el comportamiento de reciclaje de `+str_dup()+`. Pero, debido a un error tipográfico, prueba el comportamiento de una función inexistente, `+str_duq()+`. El código arroja un error y, por lo tanto, la prueba anterior pasa, pero por el _motivo equivocado_. Debido al error tipográfico, el error real arrojado se debe a que no se puede encontrar la función `+str_duq()+`:
...
Para verificar la _ausencia_ de un error, advertencia o mensaje, use `+expect_no_error()+`:

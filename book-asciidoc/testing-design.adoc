...
Observe cómo llamadas como `+library()+`, `+options()+` y `+Sys.setenv()+` tienen un efecto persistente _después_ de una prueba, incluso cuando se ejecutan dentro de `+test_that()+`:
